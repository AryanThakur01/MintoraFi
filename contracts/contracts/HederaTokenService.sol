...
abstract contract HederaTokenService {
    ...
    // function getNonFungibleTokenInfo(address token) internal returns (int responseCode, IHederaTokenService.NonFungibleTokenInfo memory tokenInfo) {
    //     (bool success, bytes memory result) = precompileAddress.call(
    //         abi.encodeWithSelector(IHederaTokenService.getNonFungibleTokenInfo.selector, token));
